...
// - questões anuladas permanecem registradas no catálogo de metadados, mas não entram no treino pontuado;
...
const SUBJECT_RANGES = [
  [1,8,'etica'],[9,10,'filosofia'],[11,16,'constitucional'],[17,18,'humanos'],[19,20,'eleitoral'],[21,22,'internacional'],[23,24,'financeiro'],[25,29,'tributario'],[30,34,'administrativo'],[35,36,'ambiental'],[37,42,'civil'],[43,44,'eca'],[45,46,'consumidor'],[47,50,'empresarial'],[51,56,'processo-civil'],[57,62,'penal'],[63,68,'processo-penal'],[69,70,'previdenciario'],[71,75,'trabalho'],[76,80,'processo-trabalho']
];
...
export const OFFICIAL_FGV_QUESTION_TOTAL = OFFICIAL_FGV_EXAM_COUNT * 80;
export const OFFICIAL_FGV_ANNULLED = Object.freeze([
  {exam:42,question:43},
  {exam:43,question:1},
  {exam:43,question:74}
]);
export const OFFICIAL_FGV_ANNULLED_COUNT = OFFICIAL_FGV_ANNULLED.length;
export const OFFICIAL_FGV_ACTIVE_TARGET = OFFICIAL_FGV_QUESTION_TOTAL - OFFICIAL_FGV_ANNULLED_COUNT;
...
function cleanOption(value){ return String(value||'').trim().replace(/^[A-D]\s*[\)\.\-:]\s*/i,'').trim(); }
function letterIndex(letter){ return {A:0,B:1,C:2,D:3}[letter] ?? 0; }
...
function readCached(exam){
  try{if(typeof localStorage==='undefined')return null;const raw=localStorage.getItem(cacheKey(exam));const data=raw?JSON.parse(raw):null;return Array.isArray(data)&&data.length===80?data:null;}catch{return null;}
}
function writeCached(exam,data){try{if(typeof localStorage!=='undefined')localStorage.setItem(cacheKey(exam),JSON.stringify(data));}catch{/* best-effort */}}

async function fetchExam(exam){
  const cached=readCached(exam); if(cached)return cached;
  ...
  writeCached(exam,data); return data;
}
...
function normalizeExam(exam,rows){
  ...
  for(let number=1;number<=80;number++){
    const raw=byNumber.get(number); if(!raw)throw new Error(`FGV ${exam}: questão ${number} ausente na extração.`);
    const officialKey=meta.key[number-1];
    if(officialKey==='*')continue; // anulada: registrada em OFFICIAL_FGV_ANNULLED, fora do treino pontuado
    ...
    result.push({
      ...
      subject, topic:`${SUBJECT_LABELS[subject]} • prova oficial`, text, options,
      correct:letterIndex(officialKey),
      explanation:`Gabarito definitivo oficial da FGV/OAB: alternativa ${officialKey}. Questão oficial preservada sem comentário editorial automático.`,
      official:true, origin:'official-fgv', sourceType:'fgv-official', storage:'official', preserveOfficialOrder:true,
      exam:String(exam), examNumber:Number(exam), examType:'1', year:meta.year, examDate:meta.examDate, questionNumber:number,
      officialAnswerKey:officialKey, extractedAnswerKey:extractedKey||null, extractionKeyMatchesOfficial:extractedKey===officialKey,
      difficulty:'oficial-fgv', quality:'oficial-gabarito-definitivo',
      sources:[
      ...
      ingestion:{extractor:'dataset público estruturado; gabarito sobrescrito pela FGV',pinnedCommit:EXTRACTION_COMMIT,url:`${EXTRACTION_BASE}/questoes${exam}.json`}
    });
  }
  return result;
}
...
export function validateOfficialFgvConfig(){
  ...
  for(const [exam,meta] of Object.entries(OFFICIAL_FGV_EXAMS)){
    ...
    meta.key.forEach((v,i)=>{if(!['A','B','C','D','*'].includes(v))issues.push(`${exam}/Q${i+1}: gabarito inválido ${v}`);if(v==='*')annulled++;});
  ...
  if(annulled!==OFFICIAL_FGV_ANNULLED_COUNT)issues.push(`anulações: esperado ${OFFICIAL_FGV_ANNULLED_COUNT}, gabaritos registram ${annulled}`);
  return issues;
}
...
export async function getOfficialFgvQuestions({force=false}={}){
  ...
  inflight=(async()=>{
    ...
    if(all.length!==OFFICIAL_FGV_ACTIVE_TARGET)throw new Error(`Banco FGV: esperado ${OFFICIAL_FGV_ACTIVE_TARGET} questões válidas, recebido ${all.length}.`);
    const ids=new Set(all.map(q=>q.id)); if(ids.size!==all.length)throw new Error('Banco FGV: IDs duplicados.');
    memoryCache=all; return all;
  })();
...
export function clearOfficialFgvCache(){
  memoryCache=null;
  try{if(typeof localStorage!=='undefined')for(const exam of Object.keys(OFFICIAL_FGV_EXAMS))localStorage.removeItem(cacheKey(exam));}catch{/* noop */}
}
